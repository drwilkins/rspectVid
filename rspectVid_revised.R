...
p_load(ggplot2,seewave,tuneR,viridis,scales,gganimate,av,grid,tidyverse,png,warbleR,tools,devtools,ari,lubridate,furrr,purrr,tictoc)
...
processSound<-function(wav0,filter,ampThresh,crop,xLim,...){
  ...
  #Apply filters
  if(filter!=F){
    wav0=seewave::ffilter(wave=wav0,from=filter[1]*1000,to=filter[2]*1000,bandpass=F,output="Wave")}
  if(ampThresh!=0){wav0<-seewave::afilter(wav0,f=smplRt,threshold=ampThresh,plot=F,output="Wave")}
  ...
  if(!is.null(crop)){ 
   if(length(crop)==1){if(crop==F){crop <- c(0,fileDur0)}else{
     crop <- c(0,crop)}}
  #If supplied crop longer than wav, output error
   if(crop[2]>fileDur0){stop(paste0("\n**** Crop can't be longer than file duration: ",round(fileDur0,2),", though xLim can ****"))}
  }else{
  ...
  #browser()
  return(list(newWav=wav,segWavs=segWavs,wavDur=wavDur,xLim=xLim))
}#End processSound

##########
# ggSpec: helper function to create ggplot spectrogram of wave file

ggSpec<-function(wav,ovlp,wl,wn,yLim,xLim,colPal,colbins,ampTrans,plotLegend,onlyPlotSpec,isViridis,bg,fontAndAxisCol,min_dB,bgFlood,optim,timeAdj,...)
{
  # bg="#ebe834"
  ...
  spectrogram<-seewave::spectro(wav,plot=F,ovlp=ovlp,wl=wl,wn=wn,norm=F)
  #seewave::spec(normalize(wav,center=F,pcm=T),PSD=T,wl=wl,wn=wn,norm=F,plot=1)
  ...
  #Nudge all time stamps if necessary
  if(timeAdj!=0){
    df$time=df$time+timeAdj}
...
#Plot that thang
  G<-ggplot2::ggplot(df,ggplot2::aes(x=time,y=freq,z=amplitude))+xlim(xLim+timeAdj)+ylim(yLim)+
    #Labels
...
return(list(spec=G,fontAndAxisCol=fontAndAxisCol,autoFontCol=autoFontCol))
}#end
...
#Parameter descriptions:
  ...
  # filter: apply a bandpass filter? Defaults to none (F). Expects 'c(0,2)' where sound from 0 to 2kHz would be filtered out 
...
# colPal="inferno";crop=NULL;bg=NULL;filter=F;xLim=NULL;yLim=c(0,10);plotLegend=F;onlyPlotSpec=T;ampTrans=2.5;min_dB=-30;wl=512;ovlp=90;wn="blackman";specWidth=6;specHeight=2;colbins=30;ampThresh=0;bgFlood=F;fontAndAxisCol=NULL
##################

testSpec<-function(soundFile,dest_folder,outFilename,colPal="inferno",crop=NULL,bg=NULL,filter=F,xLim=NULL,yLim=c(0,10),plotLegend=F,onlyPlotSpec=T,ampTrans=1,min_dB=-30,wl=512,ovlp=90,wn="blackman",specWidth=6,specHeight=2,colbins=30,ampThresh=0,bgFlood=F,fontAndAxisCol=NULL,optim=NULL,...)
  {
  ...
  ###Multithreading not here yet
  nSegs=length(prepped$segWavs)
  if(nSegs==1){
    
  specOutList<-list(ggSpec(wav=prepped$segWavs[[1]],colPal=colPal,isViridis=isViridis,crop=crop,bg=bg,filter=filter,xLim=prepped$xLim,yLim=yLim,plotLegend=plotLegend,onlyPlotSpec=onlyPlotSpec,ampTrans=ampTrans,ampThresh=ampThresh,min_dB=min_dB,wl=wl,ovlp=ovlp,wn=wn,colbins=colbins,bgFlood=bgFlood,fontAndAxisCol = fontAndAxisCol,optim=optim,timeAdj=0))
    plot(specOutList[[1]]$spec)
 
  }else{
    # n_cores<-availableCores()-1
    # plan(multicore,workers=n_cores)
    
    timeAdjVec<-(0:(nSegs-1)) * prepped$xLim[2] #adjustment so progress is continuous across segments
    #Generate a list of spectrograms & a couple parameters
    specOutList<-lapply(1:nSegs,function(i){ggSpec(wav=prepped$segWavs[[i]],colPal=colPal,isViridis=isViridis,crop=crop,bg=bg,filter=filter,xLim=prepped$xLim,yLim=yLim,plotLegend=plotLegend,onlyPlotSpec=onlyPlotSpec,ampTrans=ampTrans,ampThresh=ampThresh,min_dB=min_dB,wl=wl,ovlp=ovlp,wn=wn,colbins=colbins,bgFlood=bgFlood,fontAndAxisCol = fontAndAxisCol,optim=optim,timeAdj=timeAdjVec[i])
      })
    plot(specOutList[[1]]$spec)
  }
 ...
 specParams=list(soundFile=soundFile,dest_folder=dest_folder,outFilename=outFilename,crop=crop,colPal=colPal,isViridis=isViridis,xLim=prepped$xLim,yLim=yLim,plotLegend=plotLegend,onlyPlotSpec=onlyPlotSpec,ampTrans=ampTrans,ampThresh=ampThresh,min_dB=min_dB,bg=bg,wl=wl,ovlp=ovlp,wn=wn,specWidth=specWidth,specHeight=specHeight,colbins=colbins,bgFlood=bgFlood,autoFontCol=specOutList[[1]]$autoFontCol,fontAndAxisCol = specOutList[[1]]$fontAndAxisCol,spec=lapply(specOutList,function(x) x$spec),newWav=prepped$newWav,segWavs=prepped$segWavs)      
 return(specParams)
}#end TestSpec
...
########
rspectVid <-function(specParams,vidName,framerate=30,highlightCol="gray50",cursorCol="white",delTemps=T,... )
{
 ...
 #Read PNG bitmap back in
  ...
  vidSegment<-{
    ggplot2::ggplot(played)+ggplot2::xlim(range_i)+ggplot2::ylim(specParams$yLim)+
      ...
      ggplot2::geom_rect(data=played,aes(xmin=xmin,ymin=ymin,xmax=xmax,ymax=ymax),fill=highlightCol,alpha=.5)+
      
      #Add cursor
      ggplot2::geom_segment(data=played,aes(x=xmin,xend=xmin,y=ymin,yend=ymax),col=cursorCol) +
  ...
  if(length(specParams$segWavs)>1){
    ...
    system(paste0(ari::ffmpeg_exec(),' -f concat -safe 0 -i "',paste0(tempdir,"mp4Segments.txt"),'" -codec copy -t ',cropFileDur3,'   -y "',paste0(tempdir,"deleteme.mp4"),'"') )
    
    #Add audio track back in (couldn't figure how to combine these steps)
    system(paste0(ari::ffmpeg_exec(),' -ss 0 -i "',paste0(tempdir,"deleteme.mp4"),'" -i "',newWavOut,'"  -c:v libx264 -force_key_frames "expr:gte(t,n_forced*3)" -map 0:v:0 -map 1:a:0 -c:a aac -ac 1 -b:a 192k -y -shortest -t ',cropFileDur,' "',vidName,'"'))
    # system(paste0(ari::ffmpeg_exec(),' -i "',paste0(tempdir,"deleteme.mp4"),'" -i "',newWavOut,'"  -map 0:v:0 -map 1:a:0 -c:v copy -c:a aac -ac 2 -b:a 192k -y "',vidName,'"'))
...
f<-list.files("/Users/mattwilkins/GDrive/GP/Units/unit-FemalesSing/Lessons/Science/OZtourAssets/Songs, both sexes/",pattern=".wav",full.names=T) 
lapply(f,function(x){
  t<-testSpec(x,crop=F,xLim=3,onlyPlotSpec = F,bgFlood=T)  
  rspectVid(t)
})
